...
def _build_events_compaction_config() -> EventsCompactionConfig | None:
    ...
    enabled = _parse_enabled(
        os.getenv("SENTIENTAGENT_V2_COMPACTION_ENABLED"),
        default=True,
    )
    if not enabled:
        return None
    ...
    interval = _parse_non_negative_int(
        os.getenv("SENTIENTAGENT_V2_COMPACTION_INTERVAL"),
        default=8,
    )
    overlap = _parse_non_negative_int(
        os.getenv("SENTIENTAGENT_V2_COMPACTION_OVERLAP"),
        default=1,
    )
    ...
    token_threshold = _parse_positive_int(os.getenv("SENTIENTAGENT_V2_COMPACTION_TOKEN_THRESHOLD"))
    event_retention_size = _parse_non_negative_int(
        os.getenv("SENTIENTAGENT_V2_COMPACTION_EVENT_RETENTION"),
        default=-1,
    )
