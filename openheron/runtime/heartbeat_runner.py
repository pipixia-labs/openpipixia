...
class HeartbeatRunner:
    ...
    def __init__(
        ...
        *,
        ...
        self._every_raw = every if every is not None else os.getenv("OPENHERON_HEARTBEAT_EVERY", "30m")
        self._interval_ms = parse_heartbeat_every_ms(self._every_raw)
        self._prompt = resolve_heartbeat_prompt(prompt if prompt is not None else os.getenv("OPENHERON_HEARTBEAT_PROMPT"))
        raw_active_hours = active_hours if active_hours is not None else {
            "start": os.getenv("OPENHERON_HEARTBEAT_ACTIVE_HOURS_START", "").strip(),
            "end": os.getenv("OPENHERON_HEARTBEAT_ACTIVE_HOURS_END", "").strip(),
            "timezone": os.getenv("OPENHERON_HEARTBEAT_ACTIVE_HOURS_TIMEZONE", "user").strip() or "user",
        }
