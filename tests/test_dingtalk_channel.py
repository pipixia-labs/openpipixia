...
from openppx.channels.dingtalk import DingTalkChannel
...
class DingTalkChannelTests(unittest.IsolatedAsyncioTestCase):
    async def test_send_posts_message_when_token_available(self) -> None:
        ...
        api_call.assert_awaited_once_with(
            ...
            payload={
                ...
                "msgParam": '{"text":"hello dingtalk","title":"openppx reply"}',
            },
    ...
    async def test_start_initializes_stream_client_when_available(self) -> None:
        ...
        with (
            patch("openppx.channels.dingtalk.DINGTALK_AVAILABLE", True),
            patch("openppx.channels.dingtalk.Credential", _FakeCredential),
            patch("openppx.channels.dingtalk.DingTalkStreamClient", _FakeStreamClient),
            patch("openppx.channels.dingtalk.ChatbotMessage", _FakeChatbotMessage),
        ):
            await channel.start()
    ...
    async def test_process_stream_payload_uses_chatbot_message_parser(self) -> None:
        ...
        with patch("openppx.channels.dingtalk.ChatbotMessage", _FakeChatbotMessage):
